// app/organization/[orgSlug]/houses/[houseSlug]/events/[eventId]/page.tsx
...
interface EventDetailPageProps {
  params: Promise<{ orgSlug: string; houseSlug: string; eventId: string }>
}

export default async function EventDetailPage({ params }: EventDetailPageProps) {
  const session = await getServerSession(authOptions)
  const { orgSlug, houseSlug, eventId } = await params
  ...
  const hasTickets = existingTickets.length > 0
  ...
  const getStatusConfig = () => {
    switch (event.status) {
      case "PUBLISHED":
        return { label: "Published", color: "bg-green-100 text-green-800", icon: CheckCircle }
      case "DRAFT":
      ...
      default:
        return { label: event.status, color: "bg-gray-100 text-gray-800", icon: AlertCircle }
    }
  }
  ...
  const shareUrl = `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/portal/${orgSlug}/events/${event.id}`

  return (
    <div className="container max-w-7xl mx-auto px-4 py-8">
      {/* Row 1: 3 columns - Image | Event Info | Actions */}
        ...
        <div className="lg:col-span-1 space-y-5">
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <Badge className="bg-primary/10 text-primary hover:bg-primary/20 px-3 py-1 inline-flex items-center gap-1">
                {getEventTypeIcon()}
                {getEventTypeLabel()}
              </Badge>
          ...
          <div className="sticky top-6 space-y-4">
            <Card>
              ...
              <CardContent className="space-y-3">
                <Link href={`/organization/${orgSlug}/houses/${houseSlug}/events/${eventId}/edit`}>
                ...
                {hasTickets ? (
                  <Link href={`/organization/${orgSlug}/houses/${houseSlug}/tickets?eventId=${event.id}`}>
                    <Button className="w-full" variant="outline">
                  ...
                  <Link href={`/organization/${orgSlug}/houses/${houseSlug}/tickets/create?eventId=${event.id}`}>
                    <Button className="w-full" variant="outline">
                ...
                <ShareButton url={shareUrl} variant="outline" size="default" className="w-full" />
              </CardContent>
            </Card>
          </div>
        </div>
        ...
        <Card className="shadow-sm hover:shadow-md transition-shadow">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex justify-between items-center pt-2 border-t">
              ...
              <span className="text-sm">{format(new Date(event.createdAt), "MMM d, yyyy")}</span>
            </div>
          </CardContent>
        </Card>
      </div>
