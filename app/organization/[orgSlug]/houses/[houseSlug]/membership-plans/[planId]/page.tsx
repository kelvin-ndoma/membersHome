// app/organization/[orgSlug]/houses/[houseSlug]/membership-plans/[planId]/page.tsx
...
interface PlanPageProps {
  params: Promise<{ orgSlug: string; houseSlug: string; planId: string }>
}

export default async function MembershipPlanDetailPage({ params }: PlanPageProps) {
  const session = await getServerSession(authOptions)
  const { orgSlug, houseSlug, planId } = await params
  ...
  // Get members who have this plan - using include to get user data
  const membersWithPlan = await prisma.membershipItem.findMany({
    where: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href={`/organization/${orgSlug}/houses/${houseSlug}/membership-plans/${planId}/edit`}>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Link href={`/organization/${orgSlug}/houses/${houseSlug}`} className="font-medium text-primary hover:underline flex items-center gap-1">
            ...
            <div className="space-y-3">
              {membersWithPlan.map((member) => (
                <div key={member.id} className="flex items-center justify-between border-b pb-3 last:border-0">
                  <div>
                    <p className="font-medium">{(member.user as any)?.name || (member.user as any)?.email || "Unknown"}</p>
                    <p className="text-sm text-muted-foreground">{(member.user as any)?.email}</p>
                  </div>
                  <Badge variant="outline">
                    Since {format(new Date(member.startDate), "MMM d, yyyy")}
                  </Badge>
                </div>
              ))}
            </div>
